...
static CGFloat keyBoardToolbarHeight = 35.0f;
...
- (void)checkBarButton:(id)textField {
    ...
    previousBarItem.tintColor=[UIColor whiteColor];
...
- (id)initWithControllerDelegate:(id <UIKeyboardViewControllerDelegate>)delegateObject {
	if (self = [super init]) {
        ...
        if ([self.boardDelegate isKindOfClass:[UIViewController class]]) {
			objectView = [[(UIViewController *)[self boardDelegate] view] viewWithTag:101];
		}
