...
#define FIRST_PAYITEM_TAG     111111


@interface GZShopAlertView ()
...
@property (nonatomic,strong) NSArray *dataSources;//可支付的类型

@property (nonatomic,assign) BOOL didlayPayitem;

@property (nonatomic,copy) NSString *payitmeType;//选择的支付类型id

@end

@implementation GZShopAlertView
...
- (void)showInPresentedViewController:(UIViewController *)currentController paymentItems:(nonnull NSArray *)array userDataInfo:(nonnull id)userData
{
...
- (void)updateDataUIWithModel:(id)modelObject
{
    if ([modelObject isKindOfClass:[GoodsModel class]]) {
        ...
        if (!self.didlayPayitem) {
            //添加几个布局的视图
            CGFloat margin = (self.bounds.size.width - SILVER_BUTTON_WIDTH * 3. ) / 4.;
            CGFloat itemHeight = KKFitScreen(50.);
            CGFloat itemWidth = SILVER_BUTTON_WIDTH;
            
            for (NSInteger i = 0; i < self.dataSources.count; i++) {
                NSDictionary *payitemDict = self.dataSources[i];
                GZShopAlertBtn * btn = [GZShopAlertBtn shopAlertBtnToView:self normalTitle:payitemDict[kPayitemNameKey]];
                [btn addTarget:self action:@selector(clidktype:) forControlEvents:UIControlEventTouchUpInside];
                btn.tag = FIRST_PAYITEM_TAG + i ;
                if (i ==0) {
                    btn.selected = YES;
                    NSDictionary *payitemDict = [self.dataSources objectAtCheckIndex:i];
                    self.payitmeType = payitemDict[kPayitemTypeKey];
                }
                CGFloat topmargin = (i / 3) * (itemHeight + KKFitScreen(15.)) + KKFitScreen(15.);
                [btn mas_makeConstraints:^(MASConstraintMaker *make) {
                    make.left.mas_equalTo(self.mas_left).offset(margin + (i % 3) * (margin + itemWidth));
                    make.top.mas_equalTo(self.tipLabel.mas_bottom).offset(topmargin);
                    make.width.mas_equalTo(itemWidth);
                    make.height.mas_equalTo(itemHeight);
                }];
            }
            self.didlayPayitem = YES;
        }else{
            //循环遍历，所有的btn全部
            for (NSInteger i = 0; i < self.dataSources.count; i++) {
                GZShopAlertBtn * allbtn = (GZShopAlertBtn *)[self viewWithTag:FIRST_PAYITEM_TAG + i] ;
                if (i==0) {
                    allbtn.selected = YES;
                    NSDictionary *payitemDict = [self.dataSources objectAtCheckIndex:i];
                    self.payitmeType = payitemDict[kPayitemTypeKey];
...
- (void)clidktype:(id)sender
{
    ...
    ((GZShopAlertBtn *)sender).selected = YES;
    /*
     0 银币
     1 金币
     2 理财钱包
     ...
     6 购物积分
     */
    NSInteger i = ((GZShopAlertBtn *)sender).tag - FIRST_PAYITEM_TAG;
    NSDictionary *payitemDict = [self.dataSources objectAtCheckIndex:i];
    self.payitmeType = payitemDict[kPayitemTypeKey];
...
- (void)userClickSure
{
    ...
    if (self.block) {
        self.block(self.numberLabel.text, self.payitmeType);
        return;
    }
